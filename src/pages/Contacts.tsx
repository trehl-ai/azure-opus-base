...
export default function Contacts() {
  ...
  const getPrimaryCompany = (contact: (typeof contacts)[0]) => {
    const links = contact.company_contacts as Array<{ is_primary: boolean | null; company: { id: string; name: string } | null }> | null;
    ...
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-center gap-3">
        <div className="relative flex-1 min-w-0 sm:min-w-[220px] sm:max-w-sm">
          ...
          <Input value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} placeholder="Suche nach Name oder E-Mail..." className="pl-10 rounded-full min-h-[44px]" />
        </div>
      ...
      {isMobile ? (
        <div className="space-y-2">
          {isLoading ? (
            ...
            <p className="text-center text-muted-foreground py-12">Keine Contacts gefunden.</p>
          ) : (
            ...
            <TableBody>
              {isLoading ? (
                ...
                contacts.map((contact) => {
                  ...
                  const company = getPrimaryCompany(contact);
                  return (
                    <TableRow key={contact.id} className="cursor-pointer h-[52px] hover:bg-muted/50 transition-colors" onClick={() => navigate(`/contacts/${contact.id}`)}>
                      <TableCell className="text-body font-medium text-foreground">{contact.first_name} {contact.last_name}</TableCell>
