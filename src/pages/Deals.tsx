...
export default function Deals() {
  ...
  const { data: stages } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await (supabase as any).from("pipeline_stages").select("*").eq("pipeline_id", activePipelineId).order("position");
      if (error) throw error;
      return data;
    },
  ...
  const moveDealMutation = useMutation({
    mutationFn: async ({
      ...
      const { error } = await (supabase as any).from("deals").update({ pipeline_stage_id: stageId }).eq("id", dealId);
      if (error) throw error;
      return null;
    },
    onSuccess: (result) => {
