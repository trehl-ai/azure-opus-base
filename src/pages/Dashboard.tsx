...
import {
  useDashboardDeals, useDashboardProjects, useDashboardOverdueTasks,
  useDashboardMyTasks, useDashboardOpenActivities, useDashboardDefaultStages,
} from "@/hooks/queries";
...
export default function Dashboard() {
  ...
  const dealsNoActivity = useMemo(() => {
    ...
    return openDeals.filter((d) => !openActivities.has(d.id)).slice(0, isMobile ? 3 : 5);
  }, [openDeals, openActivities, isMobile]);
  ...
  const overdueDisplay = overdueTasks?.slice(0, isMobile ? 3 : undefined);
  const myTasksDisplay = myTasks?.slice(0, isMobile ? 5 : undefined);
    ...
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="rounded-2xl border border-border bg-card p-4 md:p-6">
          ...
          {overdueDisplay && overdueDisplay.length > 0 ? (
            <div className="space-y-2">
              {overdueDisplay.map((t) => {
        ...
        <div className="rounded-2xl border border-border bg-card p-4 md:p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-[15px] font-semibold text-foreground">Deals ohne Aktivität</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          {pipelineChart.length > 0 ? (
            <ResponsiveContainer width="100%" height={isMobile ? 180 : 220}>
              <BarChart data={pipelineChart} layout="vertical" margin={{ left: 0, right: 16, top: 0, bottom: 0 }}>
                ...
                <YAxis type="category" dataKey="name" width={isMobile ? 70 : 100} tick={{ fontSize: isMobile ? 10 : 12, fill: "hsl(224,15%,60%)" }} axisLine={false} tickLine={false} />
        ...
        <div className="rounded-2xl border border-border bg-card p-4 md:p-6">
          ...
          {projectChart.length > 0 ? (
            isMobile ? (
              /* Mobile: horizontal bar representation */
                ...
                {projectChart.map((e, i) => (
                  <div key={i} className="space-y-1">
                    ...
                    <div className="h-2 rounded-full bg-muted overflow-hidden">
                      <div className="h-full rounded-full" style={{ backgroundColor: e.color, width: `${Math.max(5, (e.value / Math.max(...projectChart.map(c => c.value))) * 100)}%` }} />
                    </div>
                  </div>
                ))}
              </div>
      ...
      <div className="rounded-2xl border border-border bg-card p-4 md:p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-[15px] font-semibold text-foreground">Meine Aufgaben</h2>
          {isMobile && myTasks && myTasks.length > 5 && (
            <Link to="/tasks" className="text-[12px] text-primary hover:underline">Alle anzeigen</Link>
          )}
        </div>
        {myTasksDisplay && myTasksDisplay.length > 0 ? (
          isMobile ? (
            <div className="space-y-2">
              {myTasksDisplay.map((t) => {
                const proj = t.project as { title: string } | null;
                ...
                return (
                  <div key={t.id} className="flex items-center gap-3 py-2 border-b border-border last:border-0">
                    <span className={cn("h-2 w-2 shrink-0 rounded-full", priorityBadge[t.priority ?? "medium"]?.split(" ")[0])} />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-foreground truncate">{t.title}</p>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      {t.due_date && <span className={cn("text-[11px]", overdue ? "text-destructive font-medium" : "text-muted-foreground")}>{format(new Date(t.due_date), "dd.MM")}</span>}
                      <span className={cn("rounded-full px-2 py-0.5 text-[10px] font-medium", statusBadge[t.status])}>{statusLabel[t.status] ?? t.status}</span>
                    </div>
                  </div>
                );
              })}
