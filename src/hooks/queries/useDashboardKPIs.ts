...
export function useDashboardDefaultStages() {
  return useQuery({
    ...
    queryFn: async () => {
      const { data: pipeline } = await (supabase as any).from("pipelines").select("id").eq("is_default", true).single();
      if (!pipeline) return [];
      const { data, error } = await (supabase as any).from("pipeline_stages").select("id, name, position").eq("pipeline_id", pipeline.id).order("position");
      if (error) throw error;
      return data;
    },
  });
