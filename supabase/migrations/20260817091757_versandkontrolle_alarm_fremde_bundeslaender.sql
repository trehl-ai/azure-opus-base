-- 17.08.2026 — Nachtrag: beim Umbau der Telegram-Meldung auf das alarm-Feld
-- fiel die Pruefung "fremde > 0" ersatzlos weg. Sie erkennt Mails an Empfaenger
-- ausserhalb aller heute aktiven Kampagnen — also einen Versand, der laut Plan
-- gar nicht laufen duerfte. Der Zahlenwert wird in der neuen Meldung nicht mehr
-- ausgegeben, damit war der Befund unsichtbar.
-- Die zweite entfallene Pruefung ("Tag 3 und noch 0 Klicks") wird NICHT
-- nachgezogen: die Klickdaten sind nach der Plausible-Recherche vom 17.08.
-- nicht belastbar genug, ein Alarm darauf produziert im unbeaufsichtigten
-- Betrieb nur Rauschen.
...
tag as (
  select
    ...
    coalesce((select sum(jl.n) from je_land jl
              where not exists (select 1 from k where k.bundesland = jl.bundesland)), 0) as fremde,
...
alarm as (
  select array_remove(array[
    ...
    case when t.fremde > 0
         then 'ROT: ' || t.fremde || ' Mails an Bundeslaender ausserhalb aller heute aktiven Kampagnen' end,
