-- RPC Domain-Throttle (max 2/Domain). Spaeter abgeloest von 065808. Body-only, kein NOTIFY. Angewandt 2026-06-30.
...
AS $function$
  WITH cand AS (
    ...
    SELECT *, row_number() OVER (PARTITION BY lower(split_part(email,'@',2))
      ORDER BY outreach_score DESC NULLS LAST, contact_id) AS domain_rn FROM cand
  )
