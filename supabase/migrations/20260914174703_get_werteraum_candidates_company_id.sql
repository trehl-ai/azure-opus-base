-- 14.09.2026: company_id in der Kandidatenausgabe.
--
-- ANLASS: Zwei lebende Deals derselben Firma mit VERSCHIEDENEN Adressen laufen
-- beide durch die Kandidatenauswahl, weil dedup_email ueber die Adresse
-- arbeitet (Clarenhof: clarenhofschule@stadt-koeln.de + info@dieschulapp.de).
-- Der Erstversand-Workflow soll solche Firmen UEBERSPRINGEN statt zwei Mails
-- zu senden. Dafuer braucht er den Firmenschluessel — bisher gab die RPC nur
-- company_name aus, und ein Namensabgleich ist genau das Muster, das die
-- sieben Namenskollisionen vom 09.08. erzeugt hat.
--
-- Abwaertskompatibel: company_id wird als LETZTE Spalte angehaengt. Alle
-- Aufrufer (Erstversand-Workflow, get_versand_konsistenz, wr_tageswaechter,
-- get_werteraum_naechste_kampagne, get_werteraum_versandkontrolle) lesen
-- benannte Spalten oder count(*). Weil sich der Rueckgabetyp aendert, ist
-- DROP + CREATE noetig; die Rechte werden explizit neu gesetzt.

DROP FUNCTION public.get_werteraum_candidates(integer, text, text, integer);

CREATE FUNCTION public.get_werteraum_candidates(p_limit integer DEFAULT 30, p_bundesland text DEFAULT NULL::text, p_segment text DEFAULT 'grundschule'::text, p_domain_cap integer DEFAULT 10)
...
GRANT EXECUTE ON FUNCTION public.get_werteraum_candidates(integer, text, text, integer) TO anon, authenticated, service_role;

COMMENT ON FUNCTION public.get_werteraum_candidates(integer, text, text, integer) IS
  'Kandidaten fuer den WerteRaum-Erstversand. Seit 11.09.2026 pruefen BEIDE Stellen mit activity_type=email auch deleted_at IS NULL — vorher war ein Soft-Delete auf eine Mail-Aktivitaet wirkungslos. Die CTE bereits_bemailt bleibt bewusst OHNE companies.deleted_at-Filter: sie ist die Sperrliste, ein Filter wuerde archivierte Firmen erneut anschreibbar machen. Seit 14.09.2026 gibt sie company_id als letzte Spalte aus, damit der Erstversand Firmen mit zwei Kandidaten verschiedener Adresse ueberspringen kann — die Dedup laeuft ueber die Adresse, nicht ueber die Firma.';