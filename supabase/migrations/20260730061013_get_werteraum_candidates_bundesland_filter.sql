-- 20260730061013_get_werteraum_candidates_bundesland_filter
-- applied out-of-band via MCP, backfill.
-- Quelle: supabase_migrations.schema_migrations (ttgvhqygmgtnjgwunuwz), md5 913a023e8248357a436204003953622b verifiziert.
-- Schema-Reload-Anweisung entfernt: gehoert nicht in eine Migrationsdatei.
-- p_bundesland ergaenzt. DROP zuerst: ein zusaetzlicher Parameter mit Default erzeugt
-- eine ZWEITE Signatur, und PostgREST kann einen Aufruf mit nur p_limit dann nicht mehr
-- eindeutig aufloesen (CLAUDE.md Regel 23, sinngemaess).
-- p_bundesland = null behaelt exakt das bisherige Verhalten (bundesweit).
-- Wichtig: der Filter greift VOR Dedup und Domain-Drossel, damit beide innerhalb des
-- Bundeslands wirken und nicht Kandidaten anderer Laender die Quote verbrauchen.
drop function if exists public.get_werteraum_candidates(integer);

create or replace function public.get_werteraum_candidates(
  p_limit integer default 30,
  p_bundesland text default null
)
returns table(contact_id uuid, first_name text, last_name text, anrede text, email text,
              company_name text, outreach_hook text, outreach_email_draft text,
              outreach_cluster text, outreach_score integer, deal_id uuid, bundesland text)
language sql
security definer
...
revoke all on function public.get_werteraum_candidates(integer, text) from public, anon;
grant execute on function public.get_werteraum_candidates(integer, text) to service_role;
