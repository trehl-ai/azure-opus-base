-- 17.09.2026: FEHLERBEHEBUNG. Die Funktion war seit dem 14.09. kaputt.
--
-- ⚠ WAS PASSIERT IST: Am 14.09. habe ich einen Block ergaenzt, der die Firma
-- des Kontakts uebernimmt. Er enthielt
--     GROUP BY cc.company_id HAVING count(*) OVER () = 1
-- Postgres verbietet Fensterfunktionen in HAVING (42P20). Die Funktion warf
-- einen Fehler, statt zu arbeiten.
--
-- ⚠ DER SCHADEN: Am 16.09. um 14:56:55Z hat Sabine Huber (sh@gspocking.de,
-- Grundschule Pocking) einen Termin gebucht — "15 Min Erstgespraech WerteRaum
-- between Thomas Timmer and Sabine Huber", heute 09:45Z, also 11:45 Berlin.
-- BEIDE n8n-Workflows liefen in den Fehler. Kein Kontakt, kein Deal, keine
-- Aktivitaet, KEIN EINTRAG IN webhook_log.
-- ⚠ DER webhook_log-INSERT STEHT AM ANFANG DERSELBEN TRANSAKTION und wurde
-- mit zurueckgerollt. Deshalb sah es aus, als waeren "keine Webhooks in 48
-- Stunden" eingegangen — ein Messartefakt, das die Diagnose fast in die
-- falsche Richtung gelenkt haette.
--
-- ⚠ WARUM ES NIEMAND MERKTE: Der Block wird nur erreicht, wenn der Buchende
-- ein NEUER Kontakt ohne Deal ist UND das Event "werteraum" heisst. Mein
-- PING-Test stieg vorher aus, die Buchungen vom 10. und 11.09. liefen ueber
-- Bestandskontakte. Der erste echte Fall nach der Aenderung war der Fehler.
-- LEHRE: Ein PING beweist, dass die Funktion antwortet — nicht, dass der
-- geaenderte Pfad laeuft. Wer einen Zweig einbaut, muss diesen Zweig testen.
--
-- DIE KORREKTUR, von Claude Code vorgeschlagen: count(DISTINCT) in einer
-- Aggregation statt einer Fensterfunktion. Liefert dieselbe Absicht — die
-- Firma nur bei Eindeutigkeit, sonst NULL.
...
BEGIN
  ...
  -- Serialisiert parallele Webhooks zur selben Buchung. Cal.com liefert jede
  -- Buchung an ZWEI Workflows, 6 ms auseinander.
  ...
  IF v_deal_id IS NULL THEN
    ...
    -- ⚠ HIER LAG DER FEHLER. Vorher: GROUP BY ... HAVING count(*) OVER () = 1
    -- Fensterfunktionen sind in HAVING verboten (42P20).
    -- Jetzt: count(DISTINCT) in einer Aggregation. Gleiche Absicht — die
    -- Firma nur bei Eindeutigkeit, sonst NULL. Raten waere schlechter als leer.
