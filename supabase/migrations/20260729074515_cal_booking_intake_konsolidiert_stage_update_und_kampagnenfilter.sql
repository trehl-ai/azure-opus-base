-- Konsolidierung der beiden Cal.com-Handler auf cal_booking_intake als Basis.
-- Uebernommen aus set_werteraum_termin: Stage-Update bestehender Deals, outreach_status,
-- sprechende Fehlerstatus fuer die Telegram-Meldung.
-- Zusaetzlich vier Korrekturen:
--  1. Idempotenz prueft BEIDE Keys (cal_uid und calcom_booking_uid) und deleted_at IS NULL.
--     Ohne das entstehen erneut Doppel-Aktivitaeten wie am 29.07. (72 ms auseinander).
--  2. Deal-Suche bevorzugt die WerteRaum-Pipeline. Vorher wurde der juengste Deal des Kontakts
--     genommen, egal aus welcher Pipeline - bei einem Schulkontakt mit Corporate-Deal der falsche.
--  3. Neuanlage nur noch bei WerteRaum-Buchungen (eventTypeId oder Titel). Sonst wird der Kontakt
--     angelegt, aber KEIN Deal in der Kampagnen-Pipeline. Beleg fuer das Problem: Sandra Even
--     (gmx.de) landete am 14.07. als Schul-Deal auf "Terminiert".
--  4. Rueckgabe enthaelt zusaetzlich 'ok' (boolean), damit der n8n-IF-Node darauf pruefen kann.
...
BEGIN
  ...
  -- Gehoert die Buchung zur WerteRaum-Kampagne?
  ...
  IF v_trigger IN ('BOOKING_CANCELLED','BOOKING_REJECTED') THEN
    ...
    UPDATE deal_activities
     ...
     WHERE deleted_at IS NULL
       AND (metadata->>'cal_uid' = v_uid OR metadata->>'calcom_booking_uid' = v_uid)
  ...
  -- RESCHEDULED: bestehende Activity umdatieren
  ...
  -- Idempotenz gegen BEIDE Schluessel, nur aktive Aktivitaeten
  ...
  -- Deal suchen: WerteRaum-Pipeline bevorzugt, dann ueber die Firma, sonst juengster Deal
  ...
  IF v_deal_id IS NULL THEN
    -- Nur WerteRaum-Buchungen duerfen einen Deal in der Kampagnen-Pipeline erzeugen
    IF NOT v_ist_wr THEN
      ...
      VALUES (NULL, v_contact_id,'meeting','📅 '||v_title||' (Cal.com, ausserhalb WerteRaum)',
        ...
        jsonb_build_object('cal_uid',v_uid,'calcom_booking_uid',v_uid,'trigger',v_trigger,
    ...
    v_new_deal := true;
  ELSE
    -- Bestehenden Deal auf "Terminiert" ziehen (das konnte der alte Handler nicht)
  ...
  -- outreach_status nachziehen (konnte der alte Handler nicht)
  IF v_ist_wr THEN
...
'Einziger Cal.com-Handler seit 29.07.2026. Konsolidiert die frueheren zwei Pfade. set_werteraum_termin ist damit ueberfluessig.';;