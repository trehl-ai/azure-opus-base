...
AS $function$
  ...
  FROM (
    ...
    WHERE p.name ILIKE '%werteraum%' AND d.deleted_at IS NULL
      AND c.email IS NOT NULL AND c.outreach_cluster IS NOT NULL
      AND c.lead_score >= 30 AND c.outreach_status = 'pending'
