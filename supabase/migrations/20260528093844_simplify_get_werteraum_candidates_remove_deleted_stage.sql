...
-- Source: supabase_migrations.schema_migrations[version=20260528093844].statements[1]
CREATE OR REPLACE FUNCTION public.get_werteraum_candidates(p_limit integer DEFAULT 30)
 RETURNS TABLE(contact_id uuid, first_name text, last_name text, anrede text, email text, company_name text, outreach_hook text, outreach_email_draft text, outreach_cluster text, outreach_score integer, deal_id uuid)
 LANGUAGE sql
 SECURITY DEFINER
